...
- **The leakage-free stacked arm.** Blocked by the environment: the conservation module
  requires Biopython ≥ 1.80 and the laboratory machine has 1.79. The failure is
  **open** — the conservation feature silently becomes a column of zeros — so running
  it today would produce a different model from the one that generated the sealed
  results, and not comparable to them. It was not run. A guard now makes that condition
  fail closed.
...
## 9. Hardware run in progress

A 7-circuit battery was submitted to ibm_kingston and is queued at the time of writing. It is
declared here with its job identifiers rather than waited for, so that this document
does not depend on a queue:

| Role | Protein | Shots | job_id |
|---|---|---|---|
| positive control | KRAS G12C | 4000 | `d9t16s7tfhrs73dtb550` |
| secondary control | PFKP | 4000 | `d9t16s8pdb6s73e6jh9g` |
| long-corridor exploration | Hexokinase 1 | 4000 | `d9t16sgpdb6s73e6jha0` |
| repetition | KRAS G12C | 4000 | `d9t16svpemts73cu9fhg` |
| replica of run 1 | KRAS G12C | 2000 | `d9t16t7tfhrs73dtb580` |
| transport, BCR-ABL1 | BCR-ABL1 | 4000 | `d9t16tftfhrs73dtb58g` |
| negative control | KRAS G12C | 4000 | `d9t16tfpemts73cu9fig` |

The battery includes a **negative control**, a **repetition** of the positive control
and a **replica** of the earlier hardware run, so that the result can be read against
something rather than on its own. Its analysis will be sealed separately; whatever it
shows does not change any claim above, which is entirely simulation.
