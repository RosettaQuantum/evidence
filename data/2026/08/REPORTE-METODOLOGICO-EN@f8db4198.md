...
**Rosetta Quantum does not sell a quantum algorithm. It sells the machine that measures
whether a quantum algorithm is any good — and publishes the answer even when the answer
is no.**

The Cleveland Clinic challenge asks whether a quantum computer can find *allosteric
pockets*: hidden points on a protein's surface that, when touched, switch the protein off
from a distance. The challenge statement puts them as the only therapeutic route into the
85 % of disease-causing proteins that have no viable treatment today.

**What we did.** We turn each protein into a contact network between its residues, inject
a signal at the active site, and let it spread two ways: as a quantum walk and as classical
diffusion. The question is whether the quantum version reaches the hidden pocket more
strongly than the classical one.

**What we measured.** It does not. On the four challenge targets the result is a **measured
negative** (p = 0.4038 to 0.5212) — not a tie and not a near miss. Across 90 further proteins, in
leave-one-out, the quantum arm **does not reach significance** (p = 0.104), and classical
diffusion turns out to be the best of the 5 propagators we tested — chosen by our own
selector in 50 of the 90 targets.

**Why that is the product and not the failure.** Before running anything we sealed and
timestamped the metric, the predictions and the scoring program, in that order, and only
then ran the scoring against the answer. Anyone can check that the prediction precedes
the result. That
makes it **verifiable by a third party** that we did not pick the protein or the parameter
that suited us afterwards — which is exactly how good news gets manufactured in this field.
Section 4 states where that chain has a limit, because a third party should not have to
discover it.

**It also runs on real hardware.** Seven circuits on an IBM quantum processor, their
identifiers sealed before any result existed. The circuit behaves as the simulation
predicts, within **3.4 percentage points** at worst. And we found something we were not
looking for: repeat the same circuit and the measured physics does not move, while the
noise wrapped around it moves by **10.8 points** across days. From now on, every noise
figure we publish travels with its date.

**What this document does not claim.** There is no quantum advantage. Our counter of
"crossings" — cases where the quantum method beats the best classical one — stands at
**zero**, and is published that way.

**What is left.** A measuring apparatus that works, **77 sealed files across three
independent copies** — GitHub, Codeberg and a queryable database — of which **60 are
experimental runs**, and an honest verdict on a hypothesis that was ours. During this very
submission the archive recorded **2 corrections to our own published readings**, still
visible alongside the originals. That is what is on offer: not a faster result, an
instrument that reports what it finds, including against itself.
...
|---|---|
| 1 | What this is, in one page |
| 2 | What we said we would do, and what happened |
| 3 | What was predicted, and with what |
| 4 | How the prediction was blinded, and where that blinding has a limit |
| 5 | The three metrics, fixed before anything was opened |
| 6 | The result: negative |
| 7 | Why it failed — the most useful finding of the exercise |
| 8 | Coarse-graining scalability: also negative |
| 9 | What this work does NOT claim |
| 10 | What is out of scope |
| 11 | The same circuit, run twice on hardware |
| 12 | Team |
| | How to verify this document |

## 2. What we said we would do, and what happened

In July we opened this challenge with one sentence: *«We do not promise that quantum walks
find hidden allosteric sites better than classical models — we build the apparatus that
measures, honestly and reproducibly, whether and where they do, against experimentally-
anchored ground truth, with classical diffusion as the referee.»*

We built the apparatus. It measured. **The answer is negative, and classical diffusion is
the referee that won.**

- The commitment was **sealed before the result existed** — the metric, the predictions and
  the scorer were each timestamped in that order, and for myosin the ground truth did not
  exist in the tree until afterwards. Anyone can verify the sequence in the public archive.

- Across 90 proteins in leave-one-out, the quantum manager arm does not reach significance
  (p = 0.104). Classical diffusion is the strongest single propagator, and our own manager
  selects it in 50 of 90 targets.

- On the four challenge targets, the blind prediction is a **measured negative** (p =
  0.40–0.52), and the coarse-graining test is negative too.

**One promise we did not keep.** July also promised a *discrimination forecast* — an error-
barred estimate of the structural regime in which a quantum walk would begin to separate
from classical diffusion. We do not have it. We know that in this regime the classical
propagator wins; we cannot yet say where that stops being true. Naming the gap is cheaper
than implying we closed it.

A method that only reports its wins is not a measuring instrument. This report is what the
instrument returned.
...
commit `2538c47` — which precedes not only the predictions but the entire chain,
including the metric's pre-registration eight hours later. Only 9GZ1, for myosin,
arrived after.
...
**Rosetta Quantum** is the operating brand of **Blue Tuna SpA** (Chile). Solo founder-
operator.

**Nicholas Iakl Freundlich** — Punta Arenas, Chile · hello@rosettaquantum.com

Founder & CEO of **Sumeria**, an AI conversation-analytics platform, for nine years; founder
of **Yu-Track**, software for financial-services collections. Commercial Engineer and MSc,
Universidad Adolfo Ibáñez; business exchange at HEC Montréal. Entrepreneurship World Cup
finalist (>100,000 projects across 108 countries); Parallel18 and MassChallenge alumnus.

**Relevant expertise.** Nine years shipping AI and data products — the *consume-the-verdict*
side of the problem, not the *sell-the-qubit* side. This is a first quantum venture, by
design: Rosetta's product is neutral verification, and neutrality is only credible from an
operator with no stake in any quantum hardware vendor, solver, or drug-discovery platform.

**Why this team could run this.** The verification infrastructure this challenge asks for
was already built and operating before the challenge began: a reproducible hybrid harness,
and a public evidence archive that now holds **77 sealed files across three copies** —
GitHub, Codeberg and a queryable database — of which **60 are experimental runs**, each one
hash-anchored and timestamped on Bitcoin. Every number in this report resolves to one of
them. During this submission the archive recorded not only our results but **2 corrections
to our own published readings**, both still public alongside the originals. That is the
capability being offered: not a faster algorithm, an apparatus that reports what it finds,
including against itself.

## How to verify this document

The four `content_hash` values in the header are recomputed with:
...
you.** Of the archive's 77 sealed files, 47 declare `rosettaq-archive/v1` and split
across **two** different conventions that recompute differently. The verifier tries
all of them
