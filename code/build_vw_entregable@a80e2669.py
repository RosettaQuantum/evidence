...
(worst {f4(max(e2))}). The published state of the art in 2D low-rank decomposition does not
reach the threshold either — **and that part does not depend on anything we built.**
...
this model. Everything in §4 follows from it.
...
premise, which §5.1 of the challenge statement also builds on, is that a weight matrix carries
...
We lose to the well-built baseline by **{med_razon:.0f}× at the median**, and we lose to the
naive variant on every matrix. There is no subset on which we win — which is the point: a
result that held "in general, except for some cases" would invite selecting the favourable case.
...
the state of the art is quoting a superseded method — which is the failure mode this report
documents in §4, applied to the literature rather than to a benchmark.
...
## 5. Requirements we do not meet

| requirement | status |
|---|---|
| §4.1 — demonstrate a clear advantage over the classical baseline | **Not met.** We are beaten by the baseline by {med_razon:.0f}× at the median. |
| R2 — reduced parameter count at equivalent accuracy | **Not met.** |
| R13 — ≤5% accuracy drop at ≥2× compression | **Not met**, by us and by every published result we could find for this operation. |
| R4 — public code repository with a README | **Not met yet.** Built; not yet published. |
...
§5.5 states that submissions are not penalised for degradation below the threshold **provided
the trade-off is clearly characterised**. Characterising it is what this document is.

**Optional secondary objectives (§4.2):** we claim one of four, and half of it. The code
producing every number here is released under a permissive licence; **no checkpoints are
...
`reproducir.py` runs **{N_COMPROBACIONES} checks** against `esperados.json` from a clean cache: it re-fetches
all 25 matrices and recomputes every SVD. Last full run: **0 failures** on
Python {ENT["python"]}, NumPy {ENT["numpy"]}, {ENT["plataforma"]}.

All hyperparameters, random **seeds**, dataset identities and **hardware specs** are recorded
alongside the generators, and the run starts from a **clean environment with no manual
intervention**. The public code repository §5.2 requires is **not yet published**; until it is,
this section describes what runs here, not what a third party can clone.
...
sealed before the calibration ran (`RQ-PREREG-VW-001`, OpenTimestamps-anchored).
...
Qubit count, gate fidelity and shot budget (§5.3) are **not applicable and we state why**: the
