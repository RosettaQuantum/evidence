...
if not _corridas:
    ...
    _anc = [x for x in _corridas if x["anclado"]]
    w("- **The sweep artifacts are sealed** — %s — and %d of %d are anchored in Bitcoin "
      "(OpenTimestamps), so their ordering is bounded from above by a clock neither we nor "
      "you control. The seal and the anchor are separate steps by the lab and the notary; "
      "this line reports what the archive says at build time, not what we intended. "
      "**[measured]**"
      % (", ".join("`%s` (`%s…`)" % (x["file_id"], x["content_hash"][:21])
                   for x in _corridas), len(_anc), len(_corridas)))
w()
