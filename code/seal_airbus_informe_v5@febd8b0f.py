...
if "What your benchmark can detect" not in texto:
    raise SystemExit("ABORTA: falta la seccion del umbral de deteccion")
# Las seis secciones que Nicholas echo de menos comparando contra E.ON y Cleveland.
for _s in ("## Executive summary", "Results against your three expected outcomes",
           "Feasibility and resource requirements", "Expected impact",
           "Team profile and capability", "What we are asking for"):
    if _s not in texto:
        raise SystemExit("ABORTA: falta la seccion %r. El informe volveria a ser un "
                         "informe tecnico en vez de una postulacion." % _s)
# Y la identidad: una sola ciudad, un solo correo, y el conteo de postulaciones real.
if "Santiago" in texto or "nicholasiakl@gmail.com" in texto:
    raise SystemExit("ABORTA: el §12 volvio a la identidad tecleada — contradice al pie "
                     "del propio documento, que dice Punta Arenas y hello@")
import re
# --- guardias de la v5, contra los dos defectos que esta version arregla ---
# 1. «sealed runs» donde son artefactos de ocho tipos. El numero (127) siempre estuvo bien;
#    la palabra no. Un jurado que leyera nuestra propia API veria 93 corridas.
if "sealed runs" in texto:
    raise SystemExit("ABORTA: el informe dice «sealed runs» y son artefactos de varios "
                     "tipos; las corridas son 93 de 127.")
# 2. Y el defecto que ese arreglo introdujo: al enumerar los tipos a mano quedaron cinco
#    que suman 123 al lado de un total de 127. La enumeracion se deriva de los mismos globs
#    que el total, y aqui se comprueba que sume — un lector que cuente no puede encontrar
#    una contradiccion en la misma frase.
_m = re.search(r"\*\*(\d+) sealed artefacts\*\* across the whole archive — ([^—]+) —", texto)
if not _m:
    raise SystemExit("ABORTA: no encuentro la frase del conteo de artefactos con su desglose.")
_tot = int(_m.group(1))
_suma = sum(int(x) for x in re.findall(r"(\d+)\s+[a-z-]+", _m.group(2)))
if _suma != _tot:
    raise SystemExit("ABORTA: el desglose de artefactos suma %d y el total dice %d. "
                     "Una frase que se contradice sola es peor que una cifra ausente."
                     % (_suma, _tot))
import re
import glob as _g, os as _o
_n = len(_g.glob(_o.path.join(RAIZ, "Rosetta-Quantum-*-Submission")))
_pal = {1: "first", 2: "second", 3: "third", 4: "fourth"}.get(_n, "%dth" % _n)
if ("our %s quantum submission" % _pal) not in texto:
    raise SystemExit("ABORTA: el informe no dice que es la %s postulacion, y en el disco "
                     "hay %d. «Primera» nos subvalora justo donde hay que sostener que "
                     "podemos ejecutar." % (_pal, _n))
corridas = []
for sub in ("runs",):
...
NOMBRE = ("RosettaQ__REPORT__RQ-REPORT-AIRBUS-005__%s__"
          "airbus-fase-1-informe-final-v5.json" % STAMP)
doc = {"meta": {
    "file_name": NOMBRE, "file_id": "RQ-REPORT-AIRBUS-005", "type": "REPORT",
    "is_demo": False,
    "scope_note": "Version 5 del entregable de Fase 1 del track Airbus. SUPERSEDE a "
                  "RQ-REPORT-AIRBUS-004, que queda intacto y anclado — no era falso "
                  "cuando se sello.",
    "supersede": {
        "file_id": v1["meta"]["file_id"], "content_hash": v1["meta"]["content_hash"],
        "por_que": "la v4 se entrego con «Quantum-Enabled Grid Expansion Planning» —el titulo del desafio de E.ON— impreso en la pagina 1 de su 5_verificar.pdf. No estaba en el markdown: el conversor lo ponia como valor por omision cuando el llamador no pasaba titulo, y la llamada de Airbus pasaba cuatro argumentos donde el titulo es el quinto. Ninguna guardia podia verlo porque todas leen el .md y el defecto vive en el PDF. Se quito el default del conversor (ahora aborta), la llamada pasa su titulo, y la guardia de documento paso al archivo compartido y corre sobre CADA PDF del paquete. Ademas el informe decia «sealed runs» donde son artefactos de ocho tipos. Antecedente: la v3 estaba escrita como un informe tecnico y no como una postulacion: le faltaban las seis secciones que E.ON y Cleveland si tienen —resumen ejecutivo, resultados contra los tres entregables pedidos, viabilidad, impacto, equipo y que pedimos— y su §12 decia «primera postulacion cuantica» cuando es la TERCERA, con una ciudad y un correo que contradecian el pie del propio documento. Antecedente: la v2 contaba anclas —«N of M are anchored in Bitcoin»— sobre RECIBOS, no sobre bloques confirmados, y ademas un conteo caduca en cuanto el notario ancla la siguiente. Esta version describe el mecanismo y le da al lector el comando, asi que no caduca. Y suma el §6, la medicion del umbral de deteccion. Antecedente: la v1 afirmaba en su §7, con etiqueta [measured], que «the sweep "
...
print("v5 SELLADA:", doc["meta"]["content_hash"])
