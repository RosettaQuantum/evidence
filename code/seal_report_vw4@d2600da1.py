...
ESPERADOS = {
    ...
    "ENTREGABLE-VW-EN.md": "sha256:b5898790fbe38665f428efee1de29c68dcb6466edc0258fa3e5ee989ad5be896",
    "1_Rosetta-Quantum-VW-Deliverable.pdf":
        "sha256:b59750088d8430cdef0b4d624f6b075d04205968c9a32ce219d2bf6dc8823bdd",
}
# ------------------------------------------------------------------ GUARDIA 1
hashes = {}
...
# QUE bloquea y que no, y la distincion importa porque el guardia acaba de dar un falso
# rojo sobre un texto correcto. Lo que viajo dentro del paquete de Airbus fue el TITULO de
# E.ON —«Quantum-Enabled Grid Expansion Planning»—, no la palabra «E.ON». Nombrar nuestras
# entregas anteriores en la seccion de equipo es legitimo y lo hacen los otros cuatro.
#   BLOQUEA SIEMPRE: el titulo o el tema de otro desafio, aparezca donde aparezca.
#   PERMITE: el nombre pelado de la organizacion, SOLO dentro de la seccion de equipo.
TEMAS_AJENOS = ("Grid Expansion", "grid expansion planning", "fraud detection",
                "card fraud", "cardholder", "vehicle routing", "protein", "allosteric")
for tema in TEMAS_AJENOS:
    if tema.lower() in texto_pdf.lower():
        raise SystemExit("ABORTA: «%s» —tema de otro desafio— aparece en el PDF de VW. Asi "
                         "viajo el titulo de E.ON dentro del paquete de Airbus." % tema)

NOMBRES_AJENOS = ("E.ON", "HSBC", "Airbus", "Cleveland")
_i = texto_pdf.find("Team")
_equipo = texto_pdf[_i:] if _i >= 0 else ""
for otro in NOMBRES_AJENOS:
    fuera = texto_pdf.count(otro) - _equipo.count(otro)
    if fuera > 0:
        raise SystemExit("ABORTA: «%s» aparece %d vez/veces FUERA de la seccion de equipo. "
                         "Nombrar entregas anteriores en el equipo es legitimo; en el cuerpo "
                         "es una fuga." % (otro, fuera))
...
doc = {"meta": {
    ...
    "sustituye_a": {
        "file_id": "RQ-REPORT-VW-003",
        "por_que": "Nicholas reviso el PDF y noto que FALTABA LA SECCION DE EQUIPO. "
                   "Cierto: HSBC, E.ON y Airbus la llevan y VW no. Un jurado que lee cuatro "
                   "entregables nuestros ve que a uno le falta quien lo firma. Agregada con la "
                   "misma forma de los otros —Blue Tuna SpA, el lead, el correo, el trasfondo— "
                   "y con el conteo del archivo MEDIDO al generar y anclado a su commit, "
                   "porque el archivo crece y un total sin commit no se puede comprobar. "
                   "Tambien pidio el guion de reproduccion en PDF legible: el paquete lleva "
                   "ahora 4_reproduce.pdf, con el sha256 del fuente en su cabecera para que se "
                   "pueda cotejar contra el .py del repositorio publico, que sigue siendo el "
                   "ejecutable. NINGUNA de las dos la caza un guardia: la primera es una "
                   "ausencia —un guardia comprueba lo que hay— y la segunda es una decision "
                   "sobre para quien es el archivo. ",
