...
_total_sellos = sum(len(glob.glob(os.path.join(EVID, g), recursive=True)) for g in _AG)
...
w("**Why this can execute a next phase:** the verification infrastructure this challenge "
  "would need is not a plan, it is running. **%d sealed runs** across the whole archive, "
  ...
  % (_total_sellos, len(sellos_airbus), ref("reproducir")))
w()
