...
def _censo_del_archivo():
    ...
    # Se PARSEA cada blob y se lee meta.type. La version anterior contaba con un regex
    # sobre la salida concatenada y cazaba TODO `"type"` anidado en el cuerpo del JSON:
    # daba 190 donde hay 123. El test que lo destapo fue comparar contra un conteo hecho
    # de otra forma — un numero que solo se compara consigo mismo no se puede desmentir.
    raw = sp.run(["git", "cat-file", "--batch"], cwd=EV, input=("\n".join(quiero)).encode(),
    ...
    while i < len(raw):
        ...
        if len(cab) != 3: break
        n = int(cab[2])
        cuerpo = raw[j + 1:j + 1 + n]
        i = j + 1 + n + 1
        try:
            t = json.loads(cuerpo).get("meta", {}).get("type")
        except Exception:
            continue
        if t: c[t] += 1
    return c
...
w("")
w("**Rosetta Quantum · track HSBC del 2026 Global Quantum + AI Challenge**")
w("")
...
# PRECISION SOBRE COBERTURA: la primera version incluia «PENDIENTE» y «TODO», y marco dos
# falsos positivos — «TODO» es la palabra española dentro de un bloque de codigo, y
# «PENDIENTE» es un estado legitimo de la tabla REFORMS. Un falso positivo retiene trabajo
# bueno, que es peor que dejar pasar un caso. Quedan solo las frases de andamiaje que de
# verdad viajaron al PDF.
_ANDAMIO = ["NO publicado", "NOT published", "borrador para aprobación",
            "drafted for Nicholas", "— NO publicado"]
_restos = [x for x in _ANDAMIO if x in "\n".join(L)]
if _restos:
    raise SystemExit("ABORTA: queda andamiaje de borrador en el documento: %s. Llego al PDF "
                     "del paquete una vez, diciendole al jurado que era un borrador sin "
                     "aprobar." % _restos)
