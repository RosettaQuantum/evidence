...
V1 = glob.glob(os.path.join(EV, "reports", "2026", "08", "*RQ-REPORT-AIRBUS-001*.json"))[0]
v1 = json.load(open(V1)); assert rs.verify(v1)
...
NOMBRE = ("RosettaQ__REPORT__RQ-REPORT-AIRBUS-002__%s__"
          "airbus-fase-1-informe-final-v2.json" % STAMP)
doc = {"meta": {
    "file_name": NOMBRE, "file_id": "RQ-REPORT-AIRBUS-002", "type": "REPORT",
    "is_demo": False,
    "scope_note": "Version 2 del entregable de Fase 1 del track Airbus. SUPERSEDE a "
                  "RQ-REPORT-AIRBUS-001, que queda intacto y anclado — no era falso "
                  "cuando se sello.",
    "supersede": {
        "file_id": v1["meta"]["file_id"], "content_hash": v1["meta"]["content_hash"],
        "por_que": "la v1 afirma en su §7, con etiqueta [measured], que «the sweep "
...
print("v2 SELLADA:", doc["meta"]["content_hash"])
